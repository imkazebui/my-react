...
class FetchHelper {
  ...
  constructor() {
    ...
    this.addBeforeRequestInterceptor = this.addBeforeRequestInterceptor.bind(
      this
    );
    this.addAfterResonseInterceptor = this.addAfterResonseInterceptor.bind(
      this
    );
  ...
  _fetchWith401Retry = fetch => async (input, init) => {
    ...
    if (resp.status === 401) {
      console.warn(
        "TODO - [FetchHelper#_fetchWith401Retry] get refesh_token to retry"
      );
